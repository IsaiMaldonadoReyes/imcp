  ...
  <ion-page>
    <ion-content>
      ...
      <v-dialog v-model="dialogPropiedades.dialog" max-width="500px">
        hola
        <div class="icon-box">
          <lottie-animation ref="anim" :animationData="IncorrectAnimation" :loop="false" :autoPlay="true" :speed="0.5"
            class="lottie-container" />
        </div>
        <v-card class="pa-3">
          <v-card-title class="text-grey-darken-1" style="text-align: center" :color="colores.rojoIMPC">
            {{ dialogPropiedades.mensajeTitulo }}
          </v-card-title <v-toolbar flat color="transparent">
          <v-toolbar-title>
            <!--v-card-title
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn :color="colores.rojoIMPC" block size="large" variant="flat"
              @click="cerrardialogPropiedades(dialogPropiedades.correcto)">Aceptar</v-btn>
            <v-spacer></v-spacer>
          </v-card-actions>
        </v-card>
      </v-dialog>
