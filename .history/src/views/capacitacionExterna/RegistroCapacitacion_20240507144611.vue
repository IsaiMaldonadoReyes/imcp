  ...
  setup() {
    ...
    async function catalogoColegio() {
      try {
        ...
        if (
          Array.isArray(capacitacionStore.object.catalogoColegio) &&
          capacitacionStore.object.catalogoColegio.length > 0
        ) {
          dataColegio.value = { result: capacitacionStore.object.catalogoColegio };
        }
      } catch (error) {}
    }
    ...
    async function guardarDatos() {
      try {
        ...
        console.log(dataColegio.value.result.length);

        if (isValidForm.valid && dataModel.value) {
        ...
        } else if (!isValidForm.valid && dataModel.value) {
          dialogPropiedades.value = {
            dialog: true,
            titulo: "Capacitación externa",
            cuerpo:
