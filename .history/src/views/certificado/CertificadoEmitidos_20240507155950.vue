  ...
  setup() {
    ...
    async function cargarDesglosePorEjercicio() {
      ...
      try {
        ...
        console.log(certificadoStore.object.certificadosEmitidos.totalSize);
        console.log(certificadoStore.object.certificadosEmitidos.length);

        if (
          Array.isArray(certificadoStore.object.certificadosEmitidos) &&
          certificadoStore.object.certificadosEmitidos.totalSize > 0
        ) {
          certificadosEmitidos.value = {
            dataset: certificadoStore.object.certificadosEmitidos.filter(
              (certificado: Certificados) => certificado.Descargar !== ""
            ),
          };
        }
      } catch (error) {}
