            ...
            <template v-slot:header>
              <v-toolbar class="px-2">
                ...
                <v-btn-toggle class="mx-4" v-model="sortDesc">
                  <v-btn color="primary" @click="sort(true)" />
                  <v-btn color="primary" @click="sort(false)" />
                </v-btn-toggle>
              </v-toolbar>
            </template>
...
export default defineComponent({
  ...
  setup() {
    ...
    const sortBy = ref<
      Array<{ text: string; value: string; key: string; order: boolean }>
    >([]);
    ...
    const sortByArray = ref(Object.values(sortBy.value));

    function sort(value: boolean) {
      sortByArray.value.forEach((item) => {
        item.order = value;
      });
    ...
    onMounted(() => {
      sort(true); // O el valor que desees
    });

    return { search, games, sortedGames, SelectItemKey, sortBy, keys, sortDesc, sort };
  },
});
</script>
