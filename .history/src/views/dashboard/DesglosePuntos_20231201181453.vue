...
export default defineComponent({
  ...
  setup() {
    ...
    const customFilter = (items: Especialidad[]) => {

      console.log("filtrando");
      console.log(item);
      const area = item.areaEspecialidad.toLowerCase().includes(search.value.toLowerCase());
      const areas = items.filter(row => filter(row["type"], search));
      console.log(area);
    };
