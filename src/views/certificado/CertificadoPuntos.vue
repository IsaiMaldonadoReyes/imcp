    ...
    <ion-content>
      <v-container fluid>
        <v-card elevation="0" color="transparent">
          <v-card-item>
            <v-card-title class="text-grey-darken-3 text-center" style="white-space: normal">
        ...
        <v-card class="my-2" elevation="0" border>
          <v-card-text>
            <v-table density="compact">
              <tbody>
                <tr>
                  <td class="ma-1 pa-1 text-subtitle-1 text-grey-darken-1" style="line-height: 1.2em">
                    Periodo:
                  </td>
                  <td class="ma-1 pa-2 text-subtitle-1 font-weight-bold" style="line-height: 1.2em">
                ...
                <tr>
                  ...
                  <td class="ma-1 pa-1 text-subtitle-1 font-weight-bold text-justify" style="line-height: 1.2em">
                    {{ numCertificado }}
                  </td>
                </tr>
              </tbody>
        ...
        <v-card border class="my-4 py-1" elevation="0" v-for="item in certificadosPendientes.dataset"
          ...
          <v-row align="start" no-gutters>
            <v-col cols="4">
              <v-card class="ms-1 my-1 text-center"
                :color="getColor(item.puntos_principales, item.control_puntos_principales)" elevation="0" rounded="0">
                <v-card-text
                  :style="`color:
                  ${getColorFuerte(item.puntos_principales, item.control_puntos_principales)}; line-height: 1 !important; height: 55px;`">
                  <span class="text-subtitle-2 font-weight-bold" style="line-height: 1 !important">
                    Puntaje principal
                  </span>
                </v-card-text>
                <v-card-item style="line-height: 1 !important; height: 50px">
                  <span class="text-h6 text-grey-darken-1">{{ item.puntos_principales }}/{{
                    item.control_puntos_principales }}</span>
                </v-card-item>
              </v-card>
            </v-col>
            <v-col cols="4">
              <v-card class="ms-1 my-1 text-center"
                :color="getColor(item.puntos_otros_cursos_sobrantes, item.control_puntos_otros_cursos)" elevation="0"
                rounded="0">
                <v-card-text
                  :style="`color:
                  ${getColorFuerte(item.puntos_otros_cursos_sobrantes, item.control_puntos_otros_cursos)}; line-height: 1 !important; height: 55px;`">
                  <span class="text-subtitle-2 font-weight-bold" style="line-height: 1 !important">
                    Puntaje otros cursos
                  </span>
                </v-card-text>
                <v-card-item style="line-height: 1 !important; height: 50px">
                  <span class="text-h6 text-grey-darken-1">{{ item.puntos_otros_cursos_sobrantes }}/{{
                    item.control_puntos_otros_cursos }}</span>
                </v-card-item>
              </v-card>
            </v-col>
            <v-col cols="4">
              <v-card class="ms-1 me-1 my-1 text-center" :color="getColor(item.puntos_etica, item.control_puntos_etica)"
                elevation="0" rounded="0">
                <v-card-text
                  :style="`color:
                  ${getColorFuerte(item.puntos_etica, item.control_puntos_etica)}; line-height: 1 !important; height: 55px;`">
                  <span class="text-subtitle-2 font-weight-bold" style="line-height: 1 !important">
                    Puntaje ética
                  </span>
                </v-card-text>
                <v-card-item style="line-height: 1 !important; height: 50px">
                  <span class="text-h6 text-grey-darken-1">{{ item.puntos_etica }}/{{
                    item.control_puntos_etica }}</span>
                </v-card-item>
              </v-card>
            </v-col>
          </v-row>
...
export default defineComponent({
  ...
  setup() {
    ...
    const getColor = computed(() => (value1: number, value2: number) => (value1 > value2 ? "#ECF3E4" : "#FAE6EA"));
    const getColorFuerte = computed(() => (value1: number, value2: number) => (value1 > value2 ? "#468C00" : "#B20000"));
    ...
    async function cargarPuntosPorCertificado(idCertificado: any, anhioInicio: any, anhioFin: any, numCertificadox: any) {
      try {
        await certificadoStore.cargarPuntosPorCertificado(idCertificado, anhioInicio, anhioFin);
        certificadosPendientes.value = certificadoStore.object.puntosPorCertificado as Puntos;
